...
import { Container, Segment, Message, Input, Form, Icon} from 'semantic-ui-react'
...
const Chat = ({getConversation,conversation,cableApp ,match, loading}) => {
        ...
        if (!loading) {
            ...
            }, 
            {
                ...
                createChatMessage(text){
                    ...
                    chatChannelRef.current.perform(
                        'create_message',
    ...
    return (
        <Container style={containerStyle}>
            <Segment style={{height:"100%", overflow:"scroll", display: "flex", flexDirection:"column"}}>
            {
                user && messages.length !== 0 ? (
                    messages.map((message, index) => (
                    <Fragment key={index}>
                    <Message style={message.user_id === user.id ? senderStyle : receiverStyle} color={message.user_id === user.id ? 'violet':'teal'} compact>{message.text}</Message>
                    <br />
                    </Fragment>
                ))
                ) : ""
